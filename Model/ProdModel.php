...
class ProdModel
{
    ...
    function addProduct($nom_prod, $marca, $peso, $unidad_medida, $precio, $id_cat, $nombre_archivo = null, $ruta_temporal= null)
    {
        $filePath = "img/" . uniqid("", true) . ".". strtolower(pathinfo($nombre_archivo, PATHINFO_EXTENSION));
        move_uploaded_file($ruta_temporal, $filePath);
        $query = $this->db->prepare("INSERT INTO producto (id_prod, nom_prod, marca, peso, unidad_medida, precio, id_cat, imagen) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?)");
        $query->execute(array($nom_prod, $marca, $peso, $unidad_medida, $precio, $id_cat, $filePath));
    }
    ...
    function submitEditProd($id, $nom_prod, $marca, $peso, $unidad_medida, $precio, $id_cat, $nombre_archivo = null, $ruta_temporal= null)
    {
        $filePath = "img/" . uniqid("", true) . ".". strtolower(pathinfo($nombre_archivo, PATHINFO_EXTENSION));
        move_uploaded_file($ruta_temporal, $filePath);       
